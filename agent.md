...
| `paths.py` | **统一可写路径解析**：config.json/.sessions/records/static-generated 全部从 `APP_DATA_DIR`（Android 上为 filesDir/data）或项目根目录派生，禁止在代码里硬编码路径 |
| `startup.py` | **Android 启动入口**（仅 Android）：设置 APP_BASE_DIR/APP_DATA_DIR 环境变量，启动 Flask 于 127.0.0.1:3000 |
| `routes/__init__.py` | 把 `pages_bp` 和 `api_bp` 注册到 app |
| `routes/pages.py` | 页面路由，主要负责返回 Jinja 模板和推进“建号流程” |
| `routes/api.py` | JSON API 路由，负责 LLM 事件生成、选择、存档、记录、数据管理、生图任务 |
| `llm_client.py` | OpenAI 格式 API 客户端、Prompt 构造、事件/身世/结局/生图提示词生成 |
| `game_data.py` | 静态游戏数据：属性图标与描述、天赋、性别、种族、预设世界、世界默认标签 |
| `game_utils.py` | 游戏业务函数：抽天赋、应用天赋、世界查找、记录读写、会话清理（**无内容审核**） |
| `world_tags.py` | 世界书标签的读取、合并、清理、格式化 |
| `templates/` | Jinja 页面模板，含被 `base.html` include 的公告弹窗 `_popup.html` 与数据管理页 `admin.html` |
...
| `apk-wrapper/` | 安卓 WebView 套壳工程（Chaquopy 打包 Python，原生桥 + 载入界面），详见其 `README.md` 与 `build_apk.bat`；被 `.gitignore` 排除 |
| `dev/` | 本地开发临时目录（含部署私钥），被 `.gitignore` 排除，不要提交 |

## 4. 后端启动流程

入口在 `app.py`（Android 端由 `startup.py` 设置环境变量后 import 触发同一流程）：

1. `paths.py` 模块级解析数据目录：`APP_DATA_DIR` 环境变量优先（Android 上 = filesDir/data），否则项目根目录。`config.json`/`.sessions`/`records`/`static/generated` 全部由此派生。
2. 创建 `.sessions` 目录。
3. `RAW_CONFIG = load_config()` 读取并补全 `config.json`。
4. `LLM_CONFIG = merge_config()` 读取 LLM 配置并合并环境变量。
5. 创建 Flask app，**显式指定 template_folder/static_folder**（`APP_BASE_DIR`，Android 上 = filesDir/app_assets 解压目录；桌面端默认项目根目录）。设置 secret key 与 Flask-Session：
   - `SESSION_TYPE = 'filesystem'`
   - 会话生命周期 3 小时。
6. 注册 Jinja 全局函数：`get_trait_icon` / `get_trait_desc` / `url_for_static`。
7. 注册 `after_request`：所有响应加 `Cache-Control: no-store`（APK 覆盖安装后 WebView 会残留 HTTP 缓存，必须禁用）。
8. 注册 `/static/generated/<path>` 路由：四格漫画保存在数据目录（与只读 static 分离），从 `GENERATED_DIR` 服务。
9. 创建全局 `LLMClient`，挂到 `app.llm_client` / `app.LLM_CONFIG`。
10. 注册 `before_request`：非 `/static/` 请求检查 `config.json` 的 mtime，变化时热重载（`_reload_interval = 5` 秒节流）。
11. `register_blueprints(app)` 注册页面和 API 蓝图。
12. 仅在 `__main__` 下：`cleanup_old_sessions(SESSION_DIR)`，然后 `app.run`。Android 端由 `startup.py` 直接 `app.run(host='127.0.0.1', port=3000)`。

关键位置（行号随版本变动，以 grep 为准）：

- 数据目录解析：`paths.py` 模块级
- 会话初始化 / 配置加载 / LLM 配置合并：`app.py` 顶部
- Flask app / 模板目录：`app.py` 的 `Flask(__name__, template_folder=..., static_folder=...)`
- no-store：`app.py` 的 `_no_cache` after_request
- generated 路由：`app.py` 的 `serve_generated`
- 热重载：`app.py` 的 `reload_config_if_changed` + `_before_request`
- Android 启动：`startup.py` 的 `main(files_dir)`（filesDir 由 Java 传入，不依赖 javaclass 桥）
...
**前端设置面板直接读写 `config.json`**（`/api/config` 或 Android 原生桥），不再是临时的 session override。旧的 `/api/llm-config` 路由与 `session['llm_override']` 写入路径已删除；后端读取 `override` 为 None 时走全局配置（代码兼容保留）。

生图配置见 `llm_client.py` 的 `load_image_gen_config`：
...
| GET | `/records` | `records_page` | `routes/pages.py` | 公开记录页（已中性化文案，无"公开/其他玩家"字样） |
| GET | `/about` | `about` | `routes/pages.py` | 关于页，含「数据管理」入口 |
| GET | `/admin` | `admin_page` | `routes/pages.py` | **数据管理页**：查看/编辑 config.json、列出并导出数据文件 |
...
| GET | `/api/records` | `api_records` | `routes/api.py` 尾部 | 列出公开记录元信息 |
| GET | `/api/records/<path:filename>` | `api_record_detail` | `routes/api.py` 尾部 | 读取单条记录完整 JSON |
| GET | `/api/config` | `api_config` | `routes/api.py` 尾部 | 读取 config.json（数据管理页与设置面板共用） |
| POST | `/api/config` | `api_config` | `routes/api.py` 尾部 | 保存 config.json（保留 app 段；`llm.api_base`/`llm.model` 空值兜底恢复默认） |
| GET | `/api/data-files` | `api_data_files` | `routes/api.py` 尾部 | 列出数据目录文件（config/records/图片/日志） |
| GET | `/api/data-file?path=` | `api_data_file` | `routes/api.py` 尾部 | 下载数据目录单文件（路径穿越防护） |
...
> `/api/llm-config` 已删除：前端设置面板直接读写 `/api/config`（Android 上走原生桥直写文件）。
...
### 7.6 LLM 配置读写（`/api/config`）

- **读取**：`GET /api/config` 返回 config.json 原文（`send_file`）。
- **保存**：`POST /api/config` 接收 `{llm, image_gen}`，**保留 app 段**（secret_key/host/port/debug 用户改不了），`llm.api_base`/`llm.model` 空值时兜底恢复默认，写入后 mtime 变化触发热重载（数秒内生效）。
- **Android 端不走此接口**：设置面板与数据管理页优先调用原生 JS 桥 `AndroidBridge.readConfigFile()/saveConfigFile()` 直读直写 config.json 文件（保留 app 段逻辑在 Java 侧），浏览器端才走 HTTP。
- 旧版「启用自定义 LLM」开关已移除：`llm.enabled` 保留 config 原值；`use_journal` 实验开关随设置写入 config 并在 `llm_client.py` 从 `self.config` 读取（`(override or {}).get('use_journal', self.config.get('use_journal', False))`）。
...
| `get_records_list` | `game_utils.py` | 列出公开记录，带目录 mtime 缓存 |
| `save_game_record` | `game_utils.py` | **直接写入 `records/*.json`（不做内容审核）** |
...
### 10.2 `base.html` 的 LLM 设置流程（直写 config.json）

`base.html` 前端脚本管理设置，**直接读写 config.json**（无「启用自定义 LLM」开关，`llm.enabled` 保留 config 原值）：

1. 打开弹窗：`loadSettings()` 从 config.json 回填输入框（**Android 走原生桥 `AndroidBridge.readConfigFile()` 同步读取，浏览器走 `GET /api/config`**）。空值字段按 `FIELD_DEFAULTS` 显示默认值（api_key 除外）。
2. 关闭弹窗（「应用」/「关闭」/点外部遮罩）：`saveSettingsToConfig()` 把输入框写入 config（Android 走 `AndroidBridge.saveConfigFile()` 直写文件并保留 app 段，浏览器走 `POST /api/config`）；JSON 解析失败不关闭弹窗。
3. 「从配置读取」按钮：`syncFromConfig()` 重新加载覆盖输入框，带成功/失败 toast。
4. 保存后 Flask 热重载数秒内生效（mtime 变化）。

> 字段映射注意：`llm-base-url` → `llm.api_base`（**不是 base_url**，`fieldPath()` 有单独映射）；`llm-image-*` → `image_gen.*`；其余 `llm-<name>` → `llm.<name>`。

### 10.2.1 Android 原生桥（`AndroidBridge`）

- `readConfigFile()` / `saveConfigFile()`：直读直写 `filesDir/data/config.json`（saveConfigFile 保留 app 段）。
- `httpRequest(method, path, body)`：HTTP 代理。**base.html 在 Android 环境全局包装 `window.fetch`** 走此桥（HttpURLConnection + CookieManager 维持 Flask session），浏览器环境不变——游戏核心 API、设置、公告、导出全部自动生效。
- `saveBase64File(filename, mime, base64)`：导出文件保存到系统下载目录（API 29+ MediaStore，26-28 公共下载目录）。
- WebView 下载：前端统一 `downloadBlob(blob, filename)`——WebView 环境转 base64 走桥，浏览器用 `<a download>`。
...
> 所有可写路径（config/.sessions/records/generated）由 `paths.py` 基于 `APP_DATA_DIR` 解析（Android = filesDir/data，桌面 = 项目根目录）。Android 上数据管理页（`/admin`）可查看/导出这些文件。
...
`app.run` 的参数来自 `config.json -> app`，默认 `host=0.0.0.0`、`port=3000`、`debug=False`。**Android 端例外**：`startup.py` 固定 `host='127.0.0.1', port=3000`，config.json 的 host/port 在 Android 上不生效（只监听回环，保证不暴露）。

`debug` 的默认值必须保持 `False`：Werkzeug 调试器允许通过浏览器执行任意 Python，与 `0.0.0.0` 同时开启等于把远程代码执行接口暴露到网络上。仅本机调试时建议同时把 `app.host` 设为 `127.0.0.1`。
...
- 游戏记录保存不做内容审核，直接落盘（`save_game_record`）。
