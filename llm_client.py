...
class LLMClient:
    ...
    def generate_events_batch(self, world, game_state, override=None):
        ...
        try:
            ...
            # 实验性功能：使用事件日志替代完整历史
            # 优先 session override，其次 config.json（设置面板直写 config）
            use_journal = (override or {}).get('use_journal', self.config.get('use_journal', False))
            if use_journal:
...
请评价这一生："""
    ...
    def generate_image(self, world, game_state, ending, override=None):
        ...
        try:
            ...
            # 写入本地文件（数据目录，与只读 static 分离）
            img_dir = GENERATED_DIR
