...
| 自定义 | 自由设定 | 自由设定名称、描述、属性和世界规则 |

蔚蓝档案支持章节选择，可进入阿拜多斯篇或游戏开发部篇等独立剧情线，使用「周」作为时间单位，结局为任务型。
...
LLM 在生成事件时会同步维护人物关系网。你遇到的每个人物都有姓名、关系类型和好感度，关系会随你的选择动态变化。
...
每个世界有一套多维标签（社会结构、自然环境、经济体系、超自然、人口构成、文化面貌），你的选择会动态改变这些标签的数值，进而影响后续事件的走向。不同世界有不同的默认标签分布。
...
保存记录前会经过 LLM 内容审核，过滤违规内容后再写入文件。

### 记录可控

命运预览页可填写玩家名（限 12 字）和选择是否公开记录。公开记录会在历史记录页面展示，不公开的不会对外显示。

### 历史记录增强

历史记录详情页支持展示生成的四格漫画图片，支持按日期和分数排序，支持搜索世界名和玩家名。
