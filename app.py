...
WORLDS = [

    {
        ...
        'prompt': '你是一个人生模拟游戏的叙事者。玩家在现代都市背景下开始了新的人生。'

                  '请根据玩家的属性和当前年龄，生成一个简短的人生事件描述（50字左右）。',
...

    {
        ...
        'prompt': '你是一个修仙小说的叙事者。玩家在九州仙域开始了修仙之路。'

                  '请根据玩家的属性和当前修为/年龄，生成一个简短的修仙事件描述（50字左右）。',
...

    {
        ...
        'prompt': '你是一个基于《蔚蓝档案》世界观的故事生成器。世界观设定：舞台是基沃托斯由数千个学园组成的巨型学园都市。核心舞台是千年科技学院，故事聚焦于游戏开发部。困境是游戏开发部面临废部，开发的游戏被评为年度最烂第一名，唯一生路是在社团大赛上获奖。你是夏莱的老师（Sensei），担任游戏制作顾问。配角：花冈柚子、才羽桃井、才羽绿、天童爱丽丝。叙事用第一人称「我」展开，节奏轻快，允许游戏术语梗。每个事件附带属性变化trait_changes（勇气/谋略/战力/体质），范围-2到+2。故事自然结束时finished设为true，epitaph写任务总结。',
...
6. 最重要规则——你必须严格参考历史记录中的「上次选择」：

   - 如果用户上次选了「自杀」「赴死」「牺牲」「放手」「放弃生命」等自我毁灭选项

   → 本批第一个事件就是死亡场景，finished设为true，游戏结束

   - 如果用户上次选了高风险选项 → 承担后果（受伤、入狱、死亡等）

   - 不要用「奇迹」「得救」「巧合」来逃避用户选择的后果
...
- 用户上次选了自杀/赴死等同归于尽的选项 → finished=true，立即死亡
