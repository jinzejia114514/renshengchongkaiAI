        ...
        try:
            ...
            if history:
                ...
                for idx, record in enumerate(history, 1):
                    history_text += f"{record.get('year', idx)}岁：{record.get('event', '')}\n"
    ...
    def generate_ending_evaluation(self, world, game_state, override=None):
        ...
        try:
            ...
            for record in history:
                history_text += f"{record.get('year', '?')}岁：{record.get('event', '')}"
                if record.get('choice'):
...
def game_traits(world_id):
    ...
    if not session.get('game', {}).get('talents'):
        return render_template('error.html',
            message='请先抽取天赋 ✦',
...
def game_preview(world_id):
    ...
    # 检查流程完整性
    if not game.get('traits') or not game.get('talents'):
        return render_template('error.html',
